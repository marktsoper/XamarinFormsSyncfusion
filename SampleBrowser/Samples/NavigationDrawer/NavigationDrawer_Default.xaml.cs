...
{
	public partial class NavigationDrawer_Default : SamplePage
	{
		public NavigationDrawer_Default ()
		{
            ...
            {
                navigationDrawer.DrawerWidth = 180;
            }
		...
		void ListView_ItemSelected (object sender, SelectedItemChangedEventArgs e)
		{
			if (e.SelectedItem.ToString ().Equals ("Home")) {
				...
				navigationDrawer.ContentView = new MainContentView_Default().Content;
			} else if (e.SelectedItem.ToString ().Equals ("Profile")) {
				  
				navigationDrawer.ContentView = new ProfileContentView_Default().Content;
			}

			else if (e.SelectedItem.ToString ().Equals ("Inbox")) {
				
				navigationDrawer.ContentView = new Inbox_Default().Content;
			}
			else if (e.SelectedItem.ToString ().Equals ("Outbox")) {
				
				navigationDrawer.ContentView = new Outbox_Default().Content;
			}
			else if (e.SelectedItem.ToString ().Equals ("Sent")) {
				

				navigationDrawer.ContentView = new SentItems_Default().Content;
			}
			else if (e.SelectedItem.ToString ().Equals ("Draft")) {
				
				navigationDrawer.ContentView = new Draft_Default().Content;
			}
			if(Device.OS != TargetPlatform.iOS)
